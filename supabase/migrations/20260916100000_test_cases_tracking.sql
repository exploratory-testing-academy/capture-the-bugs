-- Track use of the fourth guidance button, "Test cases", the same way
-- 20260915120000_requirements_tracking.sql tracked "Requirements". No new
-- event type is needed — 'guidance' events already carry a free-form `kind`
-- in their payload, and the check constraint added there was never scoped to
-- specific kind values. This just adds the rollup column.
--
-- Apply with either:
...
select
  ...
  bool_or(e.type = 'guidance' and e.payload ->> 'kind' = 'cases')        as used_test_cases
...
comment on view public.session_summary_public is
  ...
  'user-stories/test-strategy/requirements/test-cases guidance panels were opened.';
